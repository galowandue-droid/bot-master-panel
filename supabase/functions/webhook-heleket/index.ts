...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Heleket webhook received:', payload);

    // Heleket webhook format (customize based on actual API documentation)
    // Example: { status: 'completed', transaction_id: '...', user_id: '...', amount: 100 }
    
    if (payload.status === 'completed' || payload.status === 'success') {
      const userId = payload.user_id || payload.custom_data?.user_id;
    ...
    console.error('Error in webhook-heleket:', error);
